...
function matchesByDay(date, byDay, ignorePrefix = false) {
  return byDay.some((entry) => {
    ...
    const [, nth, day] = m;
    if (WEEKDAYS[date.getDay()] !== day) return false;
    if (!nth || ignorePrefix) return true;
...
function matchesMonthDay(date, byMonthDay) {
  const lastDay = new Date(date.getFullYear(), date.getMonth() + 1, 0).getDate();
  return byMonthDay.some((d) => (d > 0 ? d === date.getDate() : lastDay + d + 1 === date.getDate()));
}
...
function matchesSetPos(candidate, rule) {
  ...
  const from = yearly
    ? new Date(candidate.getFullYear(), 0, 1)
    : new Date(candidate.getFullYear(), candidate.getMonth(), 1);
  const to = yearly
  ...
  for (let day = new Date(from); day < to; day = addDays(day, 1)) {
    ...
    if (rule.byDay && !matchesByDay(day, rule.byDay, true)) continue;
    if (rule.byMonthDay && !matchesMonthDay(day, rule.byMonthDay)) continue;
  ...
  return picked.includes(candidate.getMonth() * 100 + candidate.getDate());
}
...
function matchesRule(candidate, start, rule) {
  ...
  switch (rule.freq) {
    ...
    case 'MONTHLY': {
      ...
      if (rule.bySetPos)   return matchesSetPos(candidate, rule);
      if (rule.byMonthDay) return matchesMonthDay(candidate, rule.byMonthDay);
      if (rule.byDay)      return matchesByDay(candidate, rule.byDay);
