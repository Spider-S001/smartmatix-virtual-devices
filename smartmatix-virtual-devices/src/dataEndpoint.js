...
function create({
  ...
  for (const [name, fn] of Object.entries({
    ...
    rotatePassword, deviceTypeName, applyData })) {
  ...
  function createSession(endpointId, isHub = false) {
    const token = crypto.randomBytes(32).toString('hex');

    sessions.set(token, { endpointId, hub: isHub, expiresAt: Date.now() + SESSION_TTL_MS });
  ...
  async function handle(req, res) {
    ...
    const isHub = hub !== null && endpointId === hub.endpointId;
    const entry = isHub ? hub : registry.get(endpointId);
    ...
    if (req.method === 'POST' && action === 'session') {
      ...
      attempts.delete(key);
      const token = createSession(endpointId, isHub);
    ...
    // Geräteliste, nur über den Sammelzugang
    if (req.method === 'GET' && action === 'devices') {
      const session = resolveSession(req, endpointId);
      if (!session?.hub) return sendJSON(res, 401, { error: 'Unauthorized' });
      ...
      for (const [id, item] of registry) {
        ...
        list.push({
          ...
          friendlyName: device.friendlyName,
          deviceType:   device.deviceType,
          typeName:     deviceTypeName(device.deviceType),
          calendar:     getCalendar(device.deviceId)?.enabled === true,
        });
      }
      list.sort((a, b) => String(a.friendlyName).localeCompare(String(b.friendlyName)));
   ...
   */
  function sync(devices = [], hubAccess = null) {
    registry.clear();

    // Sammelzugang nur einrichten, wenn Kennung und Passwort vorliegen
    hub = (hubAccess && isUuid(hubAccess.endpointId) && hubAccess.password)
      ? { endpointId: hubAccess.endpointId, password: hubAccess.password, deviceId: null }
      : null;
    ...
    // Sessions verwaister Endpunkte loeschen; die des Sammelzugangs bleiben
    for (const [token, sess] of sessions) {
      if (sess.hub && hub && sess.endpointId === hub.endpointId) continue;
      if (!registry.has(sess.endpointId)) sessions.delete(token);
    }

    if (registry.size > 0) start();
    else { hub = null; stop('keine aktiven Endpunkte'); }
