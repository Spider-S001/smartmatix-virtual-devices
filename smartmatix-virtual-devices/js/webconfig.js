...
(() => {
  ...
  btnEl.addEventListener('click', async () => {
    ...
    try {
      res = await fetch(root + '/session?hl=' + HL, {
        method: 'POST',
  ...
  async function loadDeviceList() {
    let res;
    try { res = await fetch(root + '/devices?hl=' + HL, { headers: { 'X-Endpoint-Token': token } }); }
    catch { err(MSG.network); return; }
    if (!res.ok) { err(res.status === 401 ? MSG.expired : MSG.unknown); return; }

    const list = (await res.json()).devices || [];
    hubDevEl.textContent = '';
    hubPickEl.classList.remove('hidden');

    if (list.length === 0) {
      const op = document.createElement('option');
      op.textContent = T.hubNone;
      hubDevEl.appendChild(op);
      hubDevEl.disabled = true;
      return;
    }

    list.forEach((d) => {
      const op = document.createElement('option');
      op.value = d.endpointId;
      op.textContent = d.friendlyName + '  \u2013  ' + (d.typeName || d.deviceType);
      hubDevEl.appendChild(op);
    });

    hubDevEl.disabled = false;
